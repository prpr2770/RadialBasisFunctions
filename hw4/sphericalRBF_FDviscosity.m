...
% in the text book along with VISCOSITY included. 
...
% ------------------------------------------------------------------------------------------------
% ------------------------------------------------------------------------------------------------
% RBF-FD Differentiation Matrix for Power of Laplacian 
global PL; 			% Power of Laplacian
PL = zeros(N,N);
PL = sparse(PL);

gamma = 0.01;
epPL = 0.1;
plfi     = @(r) exp(-(epPL*r).^2);             % Define the RBF used for PowerOfLaplacian ( Viscosity )

% Power of Laplacian for 3D
poly0= @(r) 1;
poly1 = @(r) (4*(epPL*r)^2 - 6);
poly2 = @(r) (16*(epPL*r)^4 - 80*(epPL*r)^2 + 60);
poly3 = @(r) (64*(epPL*r)^6 - 672*(epPL*r)^4 + 1680*(epPL*r)^2 - 840);
% ------------------------------------------------------------------------------------------------
% ------------------------------------------------------------------------------------------------
...
for i = 1:N % Evaluate L at xyz(i,:) when RBF centered at xyz(j,:), j=1:m
    ...
    % Insert values into Sparse Matrix
    D(i,nbrs) = stencilWts';                   % Row i of the DM computed
	
	% ======================================================
	% Computing the Power of Laplacian
	% 1. Determine the distance of all points from 
	NBRS = xyz(nbrs,1:3); 
	CNTR = xyz(i,1:3);
	repeatCNTR = repmat(CNTR,length(nbrs),1);
	componentDifference2 = (NBRS - repeatCNTR).^2;
	nodeDistances2 = sum( componentDifference2,2); 			% Row-sum
	rPL = sqrt(nodeDistances2);
	
	% For Laplacian^3
	L_powOfLaplacian = epPL^6*poly3(rPL).* plfi(rPL);
	stencilWts_PL = Ahat\L_powOfLaplacian;
	PL(i,nbrs) = stencilWts_PL';
end

% Modify the Differentiation Matrix to include the Viscosity Computation!
D = D - gamma*PL; 	
