...
N = 10;         % Total sample points needed : Always Ensure EVEN!!
...
Condition(count) = vpa(cond(A_trn));
error(count) = vpa(max(abs(feval_tst - fvals_tst)));

% unnecessary params
Lambda(count) = vpa(max(abs(lambda)));
Fvals(count) = vpa(max(abs(fvals_trn)));
